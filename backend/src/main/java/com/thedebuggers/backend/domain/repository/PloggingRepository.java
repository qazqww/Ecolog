...
public interface PloggingRepository extends JpaRepository<Plogging, Long> {
    ...
    @Query(value = "select user_no, count(*) cnt, sum(distance) dist from plogging where ended_at BETWEEN :startDay AND :endDay AND user_no in :userList group by user_no order by dist desc, cnt desc",
            nativeQuery = true)
    <T> List<T> getRankingByFollow(List<User> userList, String startDay, String endDay, Class<T> type);

    @Query(value = "select user_no, count(*) cnt, sum(distance) dist from plogging where ended_at BETWEEN :startDay AND :endDay AND user_no in (select u.no from user u where address = :address) group by user_no order by dist desc, cnt desc",
            nativeQuery = true)
